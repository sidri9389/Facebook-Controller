...
email = "bunicipitesti63@gmail.com"
...
reconnect_delay = 45 #seconds

path = '/Desktop/Data/'
#path = ''

log_file = path + 'log.txt'
verified_users_file = path + 'users.txt'

default_run_time = 20 #minutes

#Commands and Devices dictionaries
commands = {'turn on' : 1, 'turn off' : 0, 'report' : 2, 'clear command' : 3}
alt_commands = {'💡' : 1, 'start' : 1, 'stop' : 0, 'delete command' : 3, 'summary' : 2, 'cancel' : 3}
...
def online_test():
	...
	try:
		r = urlopen('https://www.google.com/', timeout=10)
...
def cron_test():
	...
	f = open(path + 'cron_test.txt','a')
	f.write('Executed task at ' + current_time + '\n')
	f.close()
...
def christmas_lights_off(thread = None):
	GPIO.output(relay_christmas_lights,GPIO.HIGH)
	GPIO.output(relay_power,GPIO.HIGH)
...
def login_facebook():
	...
	try:
		...
		print('Trying to log on Facebook')

		session = fbchat.Session.login(email, password)
		listener = fbchat.Listener(session=session, chat_on=False, foreground=False)

		print('Logged on Facebook')
		log_event('Logged on Facebook')

		verified_users = []

		try:

			f = open(verified_users_file, 'r')

			for line in f:
				verified_users.append(line.rstrip('\n'))
		...
		for event in listener.listen():
			...
			if isinstance(event, fbchat.Disconnect):
				log_event('Disconnected from Facebook')
			...
			if isinstance(event, fbchat.MessageEvent):
				...
				if event.author.id != session.user.id:
					...
					if event.author.id not in verified_users:
						...
						for response in responses:
							...
							if response.text != hello_message and response.text != passwd_req:	
								...
								if command_password == response.text:
										...
										verified_users.append(event.author.id)

										f = open(verified_users_file,'w')
										f.write(str(event.author.id) + '\n')
										f.close()

										thread.send_text("What can I do? ")
								else:
						...
						print('Received command: ', split_message)
						...
						if command == 1 and device != -1:  
							...
							thread.send_text('Umm what?')
		
	except:
		log_event('Could not connect to Facebook')
...
def compute_stop(start_h, start_m, dur_h, dur_m):
	stop_h = int(start_h + dur_h + (start_m + dur_m) / 60)
	stop_m = int((start_m + dur_m) % 60)

	return stop_h, stop_m

#Ping Check
#nom_scheduler.add_job(online_test, 'cron', second = '*/30')

#Cron Test
nom_scheduler.add_job(cron_test, 'cron', minute = '*/1')

#Summer lights
nom_scheduler.add_job(lights_on, 'cron', month = '4,5,6,7,8,9', hour = lights_sum_h, minute = lights_sum_m)

lights_sum_stop_h, lights_sum_stop_m = compute_stop(lights_sum_h, lights_sum_m, lights_dur_h, lights_dur_m)

nom_scheduler.add_job(lights_off, 'cron', month = '4,5,6,7,8,9', hour = lights_sum_stop_h, minute = lights_sum_stop_m)


#Winter lights
nom_scheduler.add_job(lights_on, 'cron', month = '1,2,3,10,11,12', hour = lights_win_h, minute = lights_win_m)
...
#Irigation
nom_scheduler.add_job(irigation_on, 'cron', month = '4,5,6,7,8,9', hour = irigation_h, minute = irigation_m)

irigation_stop_h, irigation_stop_m = compute_stop(irigation_h, irigation_m, 0, irigation_dur_m)

nom_scheduler.add_job(irigation_off, 'cron', month = '4,5,6,7,8,9', hour = irigation_stop_h, minute = irigation_stop_m)


#Christmas Lights
nom_scheduler.add_job(christmas_lights_on, 'cron', month = '1,12', hour = lights_win_h, minute = lights_win_m)
